...
if (-not (Test-Path -LiteralPath $manifestPath)) {
    throw "installer-manifest.json nije pronadjen. Pokreni build-vhs-mp4-next-installer.ps1 prvo."
}
...
if ([string]::IsNullOrWhiteSpace($Tag)) {
    $Tag = if ([string]::IsNullOrWhiteSpace([string]$manifest.ReleaseTag)) {
        "vhs-mp4-optimizer-next-" + $Version
    }
    else {
...
if ([string]::IsNullOrWhiteSpace($Repo)) {
    $Repo = if ([string]::IsNullOrWhiteSpace([string]$manifest.Repository)) {
...
if ([string]::IsNullOrWhiteSpace($Target)) {
    $Target = if ([string]::IsNullOrWhiteSpace([string]$manifest.Branch)) {
...
if ($assets.Count -eq 0) {
    throw "Nema Avalonia Next release artefakata za upload."
}
...
- Avalonia desktop build za novu cross-platform bazu
...
$title = "VHS MP4 Optimizer Next $Version"
