...
public sealed class TimelineEditorServiceTests
{
    ...
    [Fact]
    public void Duplicate_segment_should_clone_selected_segment_after_itself_and_normalize_timeline()
    {
        var mediaInfo = new MediaInfo
        {
            SourceName = "test.avi",
        ...
        var timeline = TimelineEditorService.SplitAtPlayhead(TimelineEditorService.CreateInitial(mediaInfo), 40);
        var duplicated = TimelineEditorService.DuplicateSegment(timeline, timeline.Segments[1].Id);

        Assert.Equal(3, duplicated.Segments.Count);
        Assert.NotEqual(duplicated.Segments[1].Id, duplicated.Segments[2].Id);
        Assert.Equal(40, duplicated.Segments[1].TimelineStartSeconds);
        Assert.Equal(100, duplicated.Segments[2].TimelineStartSeconds);
        Assert.Equal(40, duplicated.Segments[1].SourceStartSeconds);
        Assert.Equal(100, duplicated.Segments[1].SourceEndSeconds);
        Assert.Equal(40, duplicated.Segments[2].SourceStartSeconds);
    ...
    [Fact]
    public void Merge_with_next_should_join_adjacent_compatible_segments()
    {
        var mediaInfo = new MediaInfo
        {
            SourceName = "test.avi",
        ...
        var timeline = TimelineEditorService.SplitAtPlayhead(TimelineEditorService.CreateInitial(mediaInfo), 40);
        var merged = TimelineEditorService.MergeSegmentWithNext(timeline, timeline.Segments[0].Id);

        Assert.Single(merged.Segments);
        Assert.Equal(TimelineSegmentKind.Keep, merged.Segments[0].Kind);
        Assert.Equal(0, merged.Segments[0].TimelineStartSeconds);
    ...
    [Fact]
    public void Extract_segment_to_range_should_preserve_left_and_right_parts_as_neighbors()
    {
        var mediaInfo = new MediaInfo
        {
            SourceName = "test.avi",
        ...
        var timeline = TimelineEditorService.CreateInitial(mediaInfo);
        var extracted = TimelineEditorService.ExtractSegmentToRange(timeline, timeline.Segments[0].Id, 10, 30);

        Assert.Equal(3, extracted.Segments.Count);
        Assert.Equal(0, extracted.Segments[0].TimelineStartSeconds);
        Assert.Equal(10, extracted.Segments[1].TimelineStartSeconds);
        Assert.Equal(30, extracted.Segments[2].TimelineStartSeconds);
        Assert.Equal(0, extracted.Segments[0].SourceStartSeconds);
        Assert.Equal(10, extracted.Segments[0].SourceEndSeconds);
        Assert.Equal(10, extracted.Segments[1].SourceStartSeconds);
        Assert.Equal(30, extracted.Segments[1].SourceEndSeconds);
        Assert.Equal(30, extracted.Segments[2].SourceStartSeconds);
        Assert.Equal(100, extracted.Segments[2].SourceEndSeconds);
    }

    [Fact]
    public void Roll_boundary_with_next_should_move_edit_point_between_adjacent_segments()
    {
        var mediaInfo = new MediaInfo
        {
            SourceName = "test.avi",
        ...
        var timeline = TimelineEditorService.SplitAtPlayhead(TimelineEditorService.CreateInitial(mediaInfo), 40);
        var rolled = TimelineEditorService.RollBoundaryWithNext(timeline, timeline.Segments[0].Id, 10);

        Assert.Equal(2, rolled.Segments.Count);
        Assert.Equal(0, rolled.Segments[0].TimelineStartSeconds);
        Assert.Equal(50, rolled.Segments[0].SourceEndSeconds);
        Assert.Equal(50, rolled.Segments[1].TimelineStartSeconds);
